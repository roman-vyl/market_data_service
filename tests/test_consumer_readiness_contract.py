...
def test_open_spec_does_not_require_event_cursor_api() -> None:
    spec = Path("openspec/specs/canonical-storage/spec.md").read_text()
    assert "event cursor endpoint" not in spec.lower()
    assert "server-owned consumer cursors" in spec
