...
- [x] 1.1 Introduce one shared exception-to-HTTP mapping used by
...
- [x] 1.2 Remove `HistoryPlanningHttpHandler._map_exception`'s
      `{error, message}`/bare `{error}` shapes in favor of the shared
      mapping.
- [x] 1.3 Rename `UnknownStreamError`'s HTTP code from `stream_not_found`
      to `configured_stream_not_found` on the bounds and continuity-audits
      endpoints.
- [x] 1.4 Rename `InvalidRange.code` (`application/consumer_read/errors.py`)
...
- [x] 1.5 Update `tests/test_consumer_read_http.py`,
...
- [x] 2.1 Add a format check for `expected_market_data_hash` (a
      ...
      comparison run.
- [x] 2.2 Add tests asserting a malformed-shape hash returns `422
...
- [x] 3.1 Remove the `start_time_ms`/`end_time_ms` request-body alias from
...
- [x] 3.2 Update the three `start_time_ms`/`end_time_ms` scenarios in
...
- [x] 3.3 Confirm no other internal caller (tests, docs, scripts) still
      relies on the legacy body shape.

## 4. Continuity-audit `market_data_hash` contract

- [x] 4.1 Add a test asserting `market_data_hash` is `null` in the JSON
      response when `is_continuous` is `False` (currently unasserted).
- [x] 4.2 Add a test asserting `market_data_hash` is a well-formed hex
      digest string when `is_continuous` is `True` and matches
      independently-computed `canonical_market_data_hash`.

## 5. OpenAPI parity

- [x] 5.1 Add `POST /v1/historical-candles` to `openapi_document()`:
      ...
      internal_error`.
- [x] 5.2 Correct the continuity-audits request schema in
      `openapi_document()` to declare only `from_ms`/`to_ms` (drop
      `start_time_ms`/`end_time_ms`).
- [x] 5.3 Add `market_data_hash` (nullable string) to the continuity-audit
      response schema in `openapi_document()`.
- [x] 5.4 Add any missing status codes to each documented path per the
      pinned mapping (e.g. `500` on continuity-audits, `422` on bounds for
      a malformed ticker/timeframe path segment).
- [x] 5.5 Add or update `test_consumer_read_http.py::test_openapi_document_is_served`
...
- [x] 6.1 Update any README/docs sections describing the continuity-audit
